...
const Vip = ({ data, title, selectedCategory, selectCategory }) => {
...
const filteredAnnouncements = data.filter((item) => {
  return item.vip === true && (sortOption !== "En yeniler" || item.new === "true");
});
  ...
  return (
    <section className="vip">
      <div className="container-premium2">
        <h2 className="h2-premium">VİP elanlar</h2>
      ...
      <div className="container-premium">
        ...
        {slicedAnnouncements.map((item, index) => {
           const img = Array.isArray(item.url) && require( `../../../images/home/${item.url[0]}`)
      ...
      <button className="allsee" onClick={() => handleShowAllData()}>
        {showAllData ? "Gizlə" : `Vip elanların hamısını göstər`}
      </button>
    </section>
  );
};
